...
function activeReferencesQuery(projectId) {
    return supabase
...
async function fetchReferences(projectId, chunks, queryCitations, limit) {
    ...
    if (chunkIds.length) {
        ...
        if (citIds.length) {
            ...
            if (refIdList.length) {
                ...
                const { data: refRows } = await activeReferencesQuery(projectId).in('id', refIdList);
    ...
    for (const cit of queryCitations || []) {
        ...
        const { data } = await activeReferencesQuery(projectId)
            .ilike('author', `${author}%`)
            .limit(limit);
        for (const r of data || []) {
            if (refIds.has(r.id)) continue;
